...
console.log("🚀 Starting data generation (HD iTunes Priority Enabled)...");
...
async function fetchFromiTunes(cleanQuery) {
  ...
  try {
    const res = await fetch(
      `https://itunes.apple.com/search?term=${encodeURIComponent(cleanQuery)}&entity=song&limit=5`,
    );
    ...
    if (data.results && data.results.length > 0) {
      ...
      // Upgrade default 100x100 thumbnail to crisp 600x600 HD Album Cover
      const hdCover =
        bestTrack.artworkUrl100?.replace("100x100bb.jpg", "600x600bb.jpg") ||
        null;
      return {
        cover: hdCover,
        artist: bestTrack.artistName || null,
        title: bestTrack.trackName || null,
      };
    }
  } catch (e) {
...
async function generateSongsData() {
  try {
    ...
    if (fs.existsSync(outputPath)) {
      try {
        ...
        const match = existingContent.match(
          /let\s+songs\s*=\s*(\[[\s\S]*?\])\s*;/,
        );

        if (match && match[1]) {
          const existingSongs = new Function("return " + match[1])();
          existingSongs.forEach((song) => {
            if (song.audio) existingSongsMap.set(song.audio, song);
          });
          console.log(`📦 Found ${existingSongsMap.size} existing songs.`);
        }
      } catch (err) {
        console.error("❌ WARNING: Failed to read existing data.js.");
      }
    }
    ...
    console.log(
      `✅ Total songs found: ${filteredResources.length}. Processing started...`,
    );
    const songsArray = [];
    ...
    for (let index = 0; index < filteredResources.length; index++) {
      ...
      if (ytArtist && !ytArtist.toLowerCase().includes("unknown")) {
        let cleanArtist = ytArtist
          .replace(/vevo|official|channel|topic|music/gi, "")
          .trim()
      ...
      if (existingSongsMap.has(audioUrl)) {
        ...
        // Auto-upgrade existing YouTube covers to iTunes HD covers if available
        if (!savedSong.cover || savedSong.cover.includes("ytimg.com")) {
          savedSong.cover = iTunesData?.cover || ytCover || savedSong.cover;
        }
      ...
      songsArray.push({
        ...
        artist: iTunesData?.artist || ytArtist || "Unknown Artist",
        // 🌟 PRIORITY FIX: iTunes HD 600x600 Cover FIRST -> YT Cover Fallback -> Unsplash Fallback
        cover:
          iTunesData?.cover ||
          ytCover ||
      ...
      console.log(`✨ Processed HD data for: "${exactSearchQuery}"`);
    }
    ...
    console.log("🎉 SUCCESS! 'js/data.js' updated with HD covers!");
  } catch (error) {
